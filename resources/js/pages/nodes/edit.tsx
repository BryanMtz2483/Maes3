...
export default function NodeEdit({ node }: NodeEditProps) {
    const { auth } = usePage().props as any;
    const [title, setTitle] = useState(node.title || '');
    const [description, setDescription] = useState(node.description || '');
    const [topic, setTopic] = useState(node.topic || '');
    const [coverImage, setCoverImage] = useState(node.cover_image || '');
    const [contentBlocks, setContentBlocks] = useState<ContentBlock[]>([]);
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [showPreview, setShowPreview] = useState(false);

    useEffect(() => {
        // Convertir contents a ContentBlocks
        if (node.contents && node.contents.length > 0) {
            const blocks: ContentBlock[] = node.contents.map(content => ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await axios.put(`/nodes/${node.node_id}`, {
                title,
                description,
                topic,
            ...
            if (response.data.success) {
                router.visit(`/nodes/${node.node_id}`);
            }
        } catch (error: any) {
            console.error('Error updating node:', error);
            alert(error.response?.data?.message || 'Error al actualizar el nodo');
        } finally {
                    ...
                    <div className="flex items-center justify-between mb-8">
                        <div className="flex items-center gap-4">
                            <Button
                                variant="ghost"
                                size="icon"
                                onClick={() => router.visit(`/nodes/${node.node_id}`)}
                            ...
                            <div>
                                ...
                                <p className="text-gray-400 text-sm mt-1">
                                    Actualiza el contenido de tu nodo
                                </p>
                            </div>
                        </div>
